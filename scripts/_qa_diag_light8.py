...
# Also render the whole window approach: create a parent window
...
win.show()
app.processEvents()
img3 = win.grab().toImage()
print("window.grab() samples:")
for y in range(gpos.y()+5, gpos.y()+btn.height(), 8):
    row = [tuple(img3.pixelColor(x, y).getRgb()[:3]) for x in [gpos.x()+5, gpos.x()+100]]
    print(f"  y={y}: {row}")