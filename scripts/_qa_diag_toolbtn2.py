"""Test QToolButton vs QPushButton backgrounds in scroll area."""
import sys
...
QPushButton[nav="true"], QToolButton[nav="true"] { min-height: 40px; text-align: left; background: transparent; border: 0; color: rgba(255,255,255,200); padding: 0 11px; border-radius: 9px; margin: 1px 4px; }
QPushButton[nav="true"]:checked, QToolButton[nav="true"]:checked { background: #FFFFFF; color: #1D4ED8; font-weight: 700; }
...
for name, cls in [("QPushButton", QPushButton), ("QToolButton", QToolButton)]:
    w, b = build(name, cls)
    app.processEvents()
    img = w.grab().toImage()
