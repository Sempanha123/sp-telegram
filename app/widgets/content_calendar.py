...
class ContentCalendarWidget(QWidget):
    ...
    def __init__(self,parent=None):
        super().__init__(parent);self._items=[];root=QHBoxLayout(self);self.calendar=QCalendarWidget();root.addWidget(self.calendar,2);side=QVBoxLayout();self.lbl_day=QLabel();self.lbl_day.setStyleSheet('font-weight:700');side.addWidget(self.lbl_day);self.list_day=QListWidget();side.addWidget(self.list_day,1);self.btn_calendar_open_campaign=QPushButton('Open Campaign');self.btn_calendar_open_campaign.setObjectName('btn_calendar_open_campaign');self.btn_calendar_edit_schedule=QPushButton('Edit Schedule');self.btn_calendar_edit_schedule.setObjectName('btn_calendar_edit_schedule');side.addWidget(self.btn_calendar_open_campaign);side.addWidget(self.btn_calendar_edit_schedule);root.addLayout(side,1);self.calendar.selectionChanged.connect(self.refresh_day);self.list_day.itemDoubleClicked.connect(lambda item:self.scheduleActivated.emit(int(item.data(32))));self.btn_calendar_open_campaign.clicked.connect(lambda:self._emit_selected(self.openCampaignRequested));self.btn_calendar_edit_schedule.clicked.connect(lambda:self._emit_selected(self.editScheduleRequested));self.refresh_day()
